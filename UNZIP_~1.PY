"""
Seafarer ZIP Extractor (scoped walk + parallel + debug logging)
================================================================
...
This version adds extra debug logging so we can see which sailor folders
contain a populated Original/ subfolder.
"""
...
def _scan_sailor_for_zips(drive_id, sailor_path, token):
    ...
    # DEBUG: log any Original/ that actually has contents
    if files:
        log.info("Original/ at %s has %d item(s): %s",
                 original_path, len(files),
...
def find_zip_files(drive_id, token):
    ...
    # DEBUG: show sailor paths containing TEST EXTRACT (case-insensitive)
    test_paths = [p for p in sailor_paths if "TEST EXTRACT" in p.upper()]
    log.info("DEBUG sailor_paths containing 'TEST EXTRACT': %s", test_paths)
...
def main():
    log.info("Seafarer ZIP Extractor (debug logging build) starting")
