...
def find_zip_files(drive_id, token):
    ...
    # Step 1: list top-level folders under /Email attachments and keep only
    # real year-month folders (start with a digit, not in EXCLUDE_NAMES, etc.).
...
def main():
    log.info("Seafarer ZIP Extractor (scoped walk + parallel, year-month filter) starting")
